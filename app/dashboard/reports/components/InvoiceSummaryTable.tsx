    ...
    <>
        ...
        {rows.map(({ invoice, gstTotal }) => (
          <article key={invoice.id} className="premium-surface rounded-2xl p-3">
            <div className="flex items-start justify-between gap-3">
              ...
              <p className="shrink-0 text-right font-bold text-emerald-600 dark:text-emerald-600">{formatMoney(invoice.totals?.grandTotal || 0)}</p>
            </div>
      ...
      <ResponsiveDataTable
        ...
        columns={[
          ...
          { key: "total", header: en.reports.total, align: "right", render: ({ invoice }) => formatMoney(invoice.totals?.grandTotal || 0), className: "font-bold text-emerald-600 dark:text-emerald-600" },
        ]}
      />
    </>
  )
