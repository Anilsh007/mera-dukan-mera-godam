...
export default function PricingSection({ compact = false }: PricingSectionProps) {
  return (
    <section className={compact ? "" : "relative overflow-hidden"}>
      <div className={compact ? "" : "absolute inset-0 bg-[radial-gradient(circle_at_top,rgba(16,185,129,0.18),transparent_42%),linear-gradient(135deg,rgba(255,255,255,0.03),transparent_45%)]"} aria-hidden="true" />
      ...
      <div className={compact ? "space-y-6" : "relative space-y-8 rounded-[32px] border border-[var(--border-card)] bg-[var(--bg-card-strong)] p-5 shadow-[var(--shadow-card)] backdrop-blur-xl sm:p-8 lg:p-10"}>
        <div className="max-w-3xl space-y-4">
          <p className="text-xs font-semibold uppercase tracking-[0.3em] text-emerald-500">{en.common.appName}</p>
          ...
          <p className="max-w-2xl text-sm leading-7 text-[var(--text-secondary)] sm:text-base">
            No bloated setups. No expensive yearly commitments. Get premium inventory & godown features at 1/3rd the price of traditional software.
          </p>
          <p className="max-w-2xl text-sm leading-7 text-[var(--text-secondary)] sm:text-base">
            Why pay thousands every year? Start managing your business for less than the cost of a daily cup of tea.
          </p>
        </div>
